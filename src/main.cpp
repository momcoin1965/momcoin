...
#if defined(NDEBUG)
# error "Momcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7e3a8cc92764771e59b0d1554f6e67d7f223625d25afdaa1f0e352e80d009699");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Momcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Momcoin Signed Message:\n";
...
{
    // Momcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Momcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Momcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Momcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Momcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Momcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x111afa7950cf37f0fd7fc3ad9439fe32c6383d76bc4b139921ee8c0c6ded9dad");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Future of Cryptocoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04731f5b91c583c0017ede34c8a8d8ea4efb54398c91868c02272e0eefef646a0a1a2bce8d700ca944821fd41677a012bb4ef4843bef8dbca74844a338e24ae2ba") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1544592848;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085944907;

        if (fTestNet)
        {
            block.nTime    = 1544592831;
            block.nNonce   = 385572170;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x482829bceaeb0659ec0b4c25f57cde4505c39c51b1db87d7fed1b8da528f2f05"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Momcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MomcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MomcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MomcoinMiner(CWallet *pwallet)
{
    printf("MomcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("momcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MomcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MomcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MomcoinMiner, pwallet));
}
